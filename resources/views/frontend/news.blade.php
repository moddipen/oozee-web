                ...
                <div class="col-lg-8 section-divided__main">
                    ...
                    @foreach($news as $new)
                        ...
                        <section class="section-md">
                            <h5><a href="{{ route('news.details', $new->slug) }}">{{ $new->title }}</a></h5>
                            <div class="row @if($i%2 == 0) {{ 'flex-md-row-reverse' }} @endif row-30">
                                ...
                                <div class="col-md-6">
                                    <a href="{{ route('news.details', $new->slug) }}">
                                        <figure class="figure">
                                            <img src="{{ $new->getFirstMediaUrl('news') ? asset($new->getFirstMediaUrl('news')) : 'public/frontend/images/typography-2-418x315.jpg' }}"
                                                 alt="" width="418" height="315"/>
                                        </figure>
                                    </a>
                                    <p><span class="post-inline__time">{{ $new->created_at->diffForHumans() }}</span>&nbsp;&nbsp;&nbsp;<span
                                                class="comment-minimal__author">by {{ $new->creator->name }}</span></p>
                                </div>
                            </div>
                        </section>
                    @endforeach
